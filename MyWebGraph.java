...
public class MyWebGraph {
	...
	public boolean addVertex(String s) {
		if(!vertexList.containsKey(s)){//&&!s.isEmpty()) { 
	...
	public boolean addEdge(String s, String t) {
		if(vertexList.containsKey(s)&&vertexList.containsKey(t)) {
			WebVertex startVertex = vertexList.get(s);
			return startVertex.addEdge(t);
		}
		return false;
	}
    ...
    public ArrayList<String> getVertices() {
    	...
    	//copy of key
    	ArrayList<String> vertexKeys = new ArrayList<String>(vertexList.keySet());
    	for(String vertice: vertexKeys) {
    		listUrls.add(vertice);
    	}
    	return listUrls;
    } 
    ...
    public ArrayList<String> getEdgesInto(String v) {
    	ArrayList<String> listEdgesIntoV = new ArrayList<String>();
    	
    	if(vertexList.containsKey(v)) { //to be faster, if not exist this v.
    		
	    	for(String url: vertexList.keySet()) { //find those keys have edges into him 
	    		if(vertexList.get(url).containsEdge(v)) {
	    			listEdgesIntoV.add(url);
	    		}
	    	}
    	}
    	return listEdgesIntoV;
    } 
